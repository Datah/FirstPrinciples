 ...
 * @author Andrew
 */
import com.andrewdroll.matrixmanipulator.SquareMatrix.Scalar;
import java.util.*;
...
public class Matrix implements RingElement, Serializable {
    ...
    public void readMatrix(double[][] rMatrix) throws MatrixOpException {
    ...
    public void copy(Matrix toCopy) throws MatrixOpException {
        this.readMatrix(toCopy.getEntries());
    }

    public Matrix returnCopy() throws MatrixOpException {
    ...
    public void addScaledCol(int c1, double constantMult, int c2) throws MatrixOpException {
        if (this.initialized() && (c1 >= 0 && c1 < columns) && (c2 >= 0 && c2 < columns)) {
            for (int i = 0; i < rows; i++) {
                entries[i][c1] += constantMult * entries[i][c2];
            }
        } else {
            throw new MatrixOpException("addScaledCol error - nonexistent column or matrix not initialized");
        }
    }

    public void multiplyCol(int c, double constantMult) throws MatrixOpException {
        if (this.initialized() && (c >= 0 && c < columns) && constantMult != 0) {
            for (int i = 0; i < rows; i++) {
                entries[i][c] *= constantMult;
            }
        } else {
            throw new MatrixOpException("multiplyCol error - nonexistent column or matrix not initialized");
        }
    }

    public Matrix transposeNew() throws MatrixOpException {
        if (this.initialized()) {
            ...
            throw new MatrixOpException("transposeNew error - matrix not initialized");
        }
    }
    ...
    public double determinant() throws MatrixOpException {
        if (isSquare()) {
            SquareMatrix squareMe = new SquareMatrix();
            squareMe.readMatrix(entries);
            double determinant = squareMe.determinant();
            return determinant;
        }else{
            throw new MatrixOpException("Error finding determinant - this matrix is not square. Wrong dimensions.");
        }
        
    }

    public void scalarDeterminantMe() throws MatrixOpException {
        if (isSquare()) {
            SquareMatrix squareMe = new SquareMatrix();
            squareMe.readMatrix(entries);
            Scalar determinant = squareMe.scalarDeterminant();
            readMatrix(determinant.getEntries());
        }else{
            throw new MatrixOpException("Error finding determinant - this matrix is not square. Wrong dimensions.");
        }
    }
    ...
    public void invert() throws MatrixOpException {
        if (isSquare()) {
            ...
            squareMe.readMatrix(entries);
            squareMe.invert();
            readMatrix(squareMe.entries);
        } else {
            throw new MatrixOpException("Error inverting - this matrix is not square. Wrong dimensions.");
        }
    }
    ...
    public void square() throws MatrixOpException {
        if (isSquare()) {
            ...
            throw new MatrixOpException("Error squaring - this matrix is not square. Wrong dimensions.");
        }
    }
    ...
    public void power(int n) throws MatrixOpException {
        if (isSquare()) {
            int currPower = 1;
            Matrix newThis = new Matrix(this.getEntries());
            while (currPower < n) {
                int currAdd = 1;
                Matrix currThis = new Matrix(newThis.getEntries());
            ...
            throw new MatrixOpException("Error executing power(n) - this matrix is not square. Wrong dimensions.");
        }
    }
    ...
    public Matrix coFactor(int r, int c) throws MatrixOpException {
        if (this.initialized()) {
            ...
            throw new MatrixOpException("coFactor error - matrix not yet initialized!");
        }
    }

    public void coFactorSelf(int r, int c) throws MatrixOpException {
        if (this.initialized()) {
            Matrix cof = new Matrix(rows - 1, columns - 1);
            int extra;
            for (int i = 0; i < rows - 1; i++) {
            ...
            readMatrix(cof.entries);
        } else {
            throw new MatrixOpException("coFactor error - matrix not yet initialized!");
        }
    }

    public void rightMultiply(Matrix M2) throws MatrixOpException {
        double currEntry;
        ...
        if (this.initialized() && M2.rows() == this.columns()) {
            ...
            throw new MatrixOpException("rightMultiply error - wrong dimensions or matrix not initialized.");
        }
    }
     ...
     */
    /**
     * Given list of previously found pivots and list of rows left, finds the
     * next pivot takes an int[2] array representing the last pivot position,
     * and an ArrayList of Integers representing legal pivot rows left
     */
    private int[] findPivot(int[] lastPivot, ArrayList<Integer> rowsLeft) {
        int[] nextPivot = {-1, -1};
        pivotColLoop:
        for (int j = lastPivot[1] + 1; j < columns; j++) {
    ...
    /**
     * find Reduced Row Echelon Form of the matrix returns void, takes no
     * arguments. Puts the current matrix in RREF form.
     *
